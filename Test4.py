...
class LoginApp:
    def __init__(self, master):
        ...
        self.model.load_state_dict(torch.load('mobilenet_train.pth', map_location=torch.device('cpu')))
        self.model.eval()
    ...
    def view_user_logs(self, user):
        ...
        # 获取并显示用户的认知负荷分数
        score = self.fetch_cognitive_load_score(user)
        if score is not None:
            self.log_text.insert(tk.END, f"\n用户 {user} 的认知负荷分数: {score:.2f}%\n")
        else:
            self.log_text.insert(tk.END, f"\n用户 {user} 的认知负荷分数: 无数据\n")
    ...
    def fetch_cognitive_load_score(self, user):
        try:
            cursor = self.connection.cursor()
            query = "SELECT score FROM userscores WHERE username = %s"
            cursor.execute(query, (user,))
            ...
            print("Error fetching cognitive load score:", e)
            return None
